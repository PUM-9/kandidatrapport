...
\begin{tikzpicture}
	\begin{axis}[
		symbolic x coords={Git, Google Docs, ShareLaTeX, Overleaf, SVN, Other},
		xtick=data
	 ]
...
% Lägg till att urvalsgruppen kanske inte ger en representativ generell bild på arbete av större dokument utan gäller nog bara mer för denna typen av projektgrupper.

% Kanske skriva om att vissa av alternativen inte valts alls och man inte fick något svar på varför man inte valde dem, t.ex. om de inte visste vad det var (typ skillnad på ShareLaTeX och Overleaf, om de inte använt ett av dom, hur det kan ha påverkat valet av svar, osv.) (också mercurial som kanske inte är jättekänt) innan eller om de är mindre bra att använda när man jobbar med större dokument.

% På frågan "Hur nöjd är du med att använda en tjänst som denna för hantering av dokument?" fick jag en del svar som jag inte "ville ha" typ Git och LaTeX, det frågade jag om i senare frågor som "Skulle du vilja byta hantering av dokument till asynkrona verktyg som Git och SVN?" och "Om du svarade ja, vilket verktyg skulle du då vilja använda?".

% Jag tog också snarlikna alternativ som inte skiljer sig så mycket så det kanske hade varit bättre att ta verktyg som skiljer sig mer (sharelatex och overleaf är typ samma sak). (kanske bara tagit mer generellt om asynkront eller synkront är bättre???).
